...
export default function ChatRoomPage() {
  ...
  const { user, socket, sendMessageChatRoom, deleteMessageChatRoom } =
    useAuth(); // Updated to use `user` instead of `userId`
  ...
  socket?.on(
    ...
    (payload: { chatRoomId: string; chat: Chat }) => {
      ...
      if (
        chatRoomId === id &&
        !chats.some((existingChat) => existingChat._id === chat._id)
      ) {
        setChats([...chats, chat]);
      }
    }
    ...
    <div className="w-[85vw] lg:w-[70vw] ">
      ...
      {!loading && !error && (
        <>
          ...
          <div className=" bg-gradient-to-white dark:bg-zinc-800 mt-4 grid gap-4 lg:gap-8 bg-slate-500 p-6 lg:p-20 rounded-2xl pb-60">
            {chats.map((chat) => {
              ...
              return (
                <div
                  key={chat._id}
                  className="bg-gradient-to-white dark:bg-zinc-700 rounded-xl p-4 lg:p-6"
                >
                  {!isCurrentUser && (
                  ...
                  <p className="text-right text-gray-400">
                    ...
                    {isCurrentUser && (
                      <button
                        onClick={(e) =>
                          deleteChat({ chatId: chat._id, chatRoomId: id })
                        }
